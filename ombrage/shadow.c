...
/*   Updated: 2025/02/11 22:25:08 by mrambelo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_self(t_nearest *current,t_fct *fct)
{
	...
	if (current->type == CYL)
	{
		...
		t = get_t_cyl(fct, get_delta(fct->pol), fct->origin,(t_cyl *)current->near_obj);
		// t_top = get_base_cyl(fct->dir, fct->origin, (t_cyl *)current->near_obj,1);
		// t_bot = get_base_cyl(fct->dir, fct->origin, (t_cyl *)current->near_obj,0);
